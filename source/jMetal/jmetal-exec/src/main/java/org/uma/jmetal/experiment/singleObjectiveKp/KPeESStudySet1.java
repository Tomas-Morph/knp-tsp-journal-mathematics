...
/**
 * Class to configure and run an elitist (mu + lambda) evolution strategy. The
 * target problem is single objective KP set1.
 * @author Mohammed Mahrach <mmahrach@ull.edu.es>
 * based on  Antonio J. Nebro templates <antonio@lcc.uma.es>
 */
public class KPeESStudySet1 {
...
//	  	String instanceSet = "A";
//	  	String instanceName = "2KP50-11";
...
//			mapOfAlgorithmsList.put("ssGA"+"-p"+popSize+"-e"+maxEval, ssGA);
			...
			Algorithm<BinarySolution> eES = new EvolutionStrategyBuilder<BinarySolution>(
					...
					new BitFlipMutation(0.2239),
					EvolutionStrategyBuilder.EvolutionStrategyVariant.ELITIST).setMaxEvaluations(maxEval)
					.setMu(1)
					.setLambda(4).build();
